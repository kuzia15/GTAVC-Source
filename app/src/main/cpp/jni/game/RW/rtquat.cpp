...
RwBool RtQuatConvertFromMatrix(RtQuat* qpQuat, const RwMatrix* const mpMatrix) {
    return CHook::CallFunction<RwBool>(g_libGTAVC + (VER_x32 ? 0x210ED0 + 1 : 0x2B6854), qpQuat, mpMatrix);
}

RtQuat* RtQuatRotate(RtQuat* quat, const RwV3d* axis, RwReal angle, RwOpCombineType combineOp) {
    return CHook::CallFunction<RtQuat*>(g_libGTAVC + (VER_x32 ? 0x211134 + 1 : 0x2B6AF8), quat, axis, angle, combineOp);
}

const RtQuat* RtQuatQueryRotate(const RtQuat* quat, RwV3d* unitAxis, RwReal* angle) {
    return CHook::CallFunction<const RtQuat*>(g_libGTAVC + (VER_x32 ? 0x2113C0 + 1 : 0x2B6D20), quat, unitAxis, angle);
}

RwV3d* RtQuatTransformVectors(RwV3d* vectorsOut, const RwV3d* vectorsIn, const RwInt32 numPoints, const RtQuat* quat) {
    return CHook::CallFunction<RwV3d*>(g_libGTAVC + (VER_x32 ? 0x2114D0 + 1 : 0x2B6E3C), vectorsOut, vectorsIn, numPoints, quat);
}

RwReal RtQuatModulus(RtQuat* q) {
    return CHook::CallFunction<RwReal>(g_libGTAVC + (VER_x32 ? 0x2115DA + 1 : 0x2B7044), q);
}
